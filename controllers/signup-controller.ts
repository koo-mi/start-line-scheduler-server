...
async function registerUser(req: Request, res: Response): Promise<Response> {
	...
	if (home_address === work_address) {
		return res.status(400).json({
			message: 'You must provide different address for your home and work.'
		});
	}

	// Check is username exist
	const isUserExist: UserData = await prisma.user.findUnique({
	...
	if (isUserExist) {
		return res
			.status(400)
			.json({ message: 'Existing username. Please try different name.' });
	}
